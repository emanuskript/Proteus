...
class SidebarWidget(QScrollArea):
    ...
    def _build_processing_section(self):
        ...
        btn_prev = QPushButton("Prev Band")
        btn_prev.setToolTip("Show previous PCA band (run PCA first)")
        btn_prev.clicked.connect(self.prev_pc_requested)
        btn_next = QPushButton("Next Band")
        btn_next.setToolTip("Show next PCA band (run PCA first)")
